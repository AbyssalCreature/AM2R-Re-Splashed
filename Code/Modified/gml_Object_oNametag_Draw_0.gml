var i, arrDraw, arrID, arrX, arrY, arrSprite, arrImage, arrA1, arrA1X, arrA1Y, arrA2, arrA2X, arrA2Y, arrA2A, arrMirror, arrArmmsl, arrRoom, arrName, arrBlend, arrFXTimer, arrRoomPrev, arrState, arrSAX, arrHealth, arrSpectator, tunnel, showName, text, offset, offsetY, offsetX, healthString, barHealthString, barHealth, tankHealth, fullTanks, tank, f, _x, _y, tankSprite, stretchMult, timerStr, arrSBall, teamColor;
teamColor = make_color_rgb(44, 195, 102)
            ...
            if ((arrRoom == room || arrRoom == 6 || arrRoom == 10 || arrRoom == 9 || arrRoom == 11 || arrRoom == 16) && instance_exists(oCharacter) && room != rm_transition)
            {
                if (arrSAX == global.sax)
                {
                    if (!arrSpectator)
                    {
                        ...
                        tank = 0
                        if ((!global.spectator) || (global.spectator && global.sax))
                            teamColor = make_color_rgb(0, 239, 183)
                        for (f = 0; f < 10; f++)
                        {
                            tank++
                            _x = (3 * floor((f / 2)))
                            ...
                            if (tank <= fullTanks)
                            {
                                ...
                                draw_sprite_ext(tankSprite, 0, ((arrX - offsetX) + _x), ((arrY - (offsetY + 7)) + _y), 1, 1, 0, teamColor, 1)
                            }
                            else
                                draw_sprite_ext(tankSprite, 0, ((arrX - offsetX) + _x), ((arrY - (offsetY + 7)) + _y), 1, 1, 0, c_white, 1)
                        }
                        draw_sprite_ext(sRHealthBar, 0, (arrX - offsetX), (arrY - (offsetY + 10)), 1, 1, 0, teamColor, 1)
                ...
                else if ((global.showHealthIndicators && (!tunnel) && (!arrSpectator) && (!arrSBall)) || (global.spectator && (!global.sax)) || (!global.saxmode))
                {
                    if (global.spectator || (!global.saxmode))
                        offsetY = 48
                    else
                        offsetY = 42
                    if (string_count("MorphBall", sprite_get_name(arrSprite)) > 0)
                    ...
                    tank = 0
                    if ((!global.spectator) || (global.spectator && global.sax))
                        teamColor = make_color_rgb(255, 65, 28)
                    for (f = 0; f < 10; f++)
                    {
                        tank++
                        _x = (3 * floor((f / 2)))
                    ...
                    if (((global.saxmode && global.spectator && (!global.sax)) || (!global.saxmode)) && showName)
                    {
                        if (arrRoom == room)
                            text = arrName
                        else if (arrRoom == 10)
